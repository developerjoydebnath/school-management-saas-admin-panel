...
import { lessonPlanStatusOptions } from "../dto/lesson-plan.dto";
import { useLessonPlanFilterOptions } from "../hooks/use-lesson-plan-filter-options";
import { LessonPlanFilter } from "./LessonPlanList";

type Props = {
	children?: React.ReactNode;
	filter: LessonPlanFilter;
	setFilter: (filter: LessonPlanFilter) => void;
};

/**
 * Every filter uses the shared FilterButton for one consistent look, matching
 * the homework/online-class filter bars. They are multi-select by default;
 * class is the exception, because section and subject scope to it and that
 * only has a single answer.
 */
function LessonPlanFilters({ filter, setFilter }: Omit<Props, "children">) {
	const t = useTranslations("LessonPlans");
	const { selectedSessionId } = useSessionStore();

	const { classOptions, sectionOptions, subjectOptions, teacherOptions } =
		useLessonPlanFilterOptions({
...
export default function LessonPlanFilterBar({ children, filter, setFilter }: Props) {
	return (
		<div>
			<FilterDesktopWrapper>
				<LessonPlanFilters filter={filter} setFilter={setFilter} />
			</FilterDesktopWrapper>
