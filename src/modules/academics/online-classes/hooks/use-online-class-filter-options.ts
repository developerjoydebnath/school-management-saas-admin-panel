...
/**
 * Option lists for the online class filter bar, pulled from the same
 * endpoints the create/edit form's selects use so the choices always agree.
 * Section and subject are scoped to the chosen class, matching the homework
 * filter bar this mirrors.
 */
export function useOnlineClassFilterOptions({
	classId,
	sessionId,
}: {
	classId?: string;
