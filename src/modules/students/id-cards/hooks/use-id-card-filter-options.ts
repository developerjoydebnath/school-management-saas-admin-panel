...
/**
 * Class/section options for the ID card filter, mirroring the same
 * endpoints and session-scoping the certificates/homework/lesson-plan
 * filter bars use — classes and sections are separate entities in this
 * schema, never embedded on the class record itself.
 */
export function useIdCardFilterOptions({
	classId,
	sessionId,
}: {
	classId?: string;
