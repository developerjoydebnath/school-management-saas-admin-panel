...
export function SubscriptionPlanEditForm({ id }: Props) {
	...
	const plan = rawData ? new SubscriptionPlanModel(rawData) : null;
	...
	if (isLoading) return <div className="text-muted-foreground p-6">{t("loading")}</div>;
	...
	return (
		<form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
			...
			<Card className="shadow-none ring-0">
				...
				<CardContent className="space-y-4">
					<div className="grid grid-cols-1 gap-4 @2xl/page:grid-cols-2">
						<InputField control={form.control} name="name" label="Plan Name" type="text" placeholder="Enter plan name" required />
						<InputField control={form.control} name="tagline" label="Tagline" type="text" placeholder="Enter tagline" />
					</div>
					<InputField control={form.control} name="description" label="Description" type="textarea" placeholder="Enter description" />
				</CardContent>
			</Card>
			...
			<Card className="shadow-none ring-0">
				...
				<CardContent className="space-y-4">
					<div className="grid grid-cols-1 gap-4 @2xl/page:grid-cols-3">
						<InputField control={form.control} name="priceBdt" label="Price (BDT)" type="number" placeholder="0" required />
						<InputField control={form.control} name="billingCycle" label="Billing Cycle" type="select" options={BILLING_CYCLE_OPTIONS} required />
						<InputField control={form.control} name="setupFeeBdt" label="Setup Fee (BDT)" type="number" placeholder="0" />
					</div>
					<div className="grid grid-cols-1 gap-4 @2xl/page:grid-cols-3">
						<InputField control={form.control} name="trialDays" label="Trial Days" type="number" placeholder="0" />
						<InputField control={form.control} name="gracePeriodDays" label="Grace Period Days" type="number" placeholder="7" />
						<InputField control={form.control} name="freeStudentLimit" label="Free Student Limit" type="number" placeholder="0" />
					</div>
				</CardContent>
			</Card>
			...
			<Card className="shadow-none ring-0">
				...
				<CardContent className="space-y-4">
					<div className="grid grid-cols-2 gap-4 @2xl/page:grid-cols-4">
						<InputField control={form.control} name="maxStudents" label="Max Students" type="number" placeholder="0" />
						<InputField control={form.control} name="maxTeachers" label="Max Teachers" type="number" placeholder="0" />
						<InputField control={form.control} name="maxStaff" label="Max Staff" type="number" placeholder="0" />
						<InputField control={form.control} name="maxClasses" label="Max Classes" type="number" placeholder="0" />
						<InputField control={form.control} name="maxSubjects" label="Max Subjects" type="number" placeholder="0" />
						<InputField control={form.control} name="maxBranches" label="Max Branches" type="number" placeholder="1" />
						<InputField control={form.control} name="storageGb" label="Storage Limit (GB)" type="number" placeholder="5" />
					</div>
				</CardContent>
			</Card>
			...
			<Card className="shadow-none ring-0">
				...
				<CardContent className="space-y-4">
					<div className="grid grid-cols-2 gap-4 @2xl/page:grid-cols-3">
						<InputField control={form.control} name="isPublic" label="Public Plan" type="switch" />
						<InputField control={form.control} name="isActive" label="Active" type="switch" />
						<InputField control={form.control} name="sortOrder" label="Sort Order" type="number" placeholder="0" />
					</div>
					<InputField control={form.control} name="notes" label="Internal Notes" type="textarea" placeholder="Add internal notes" />
				</CardContent>
			</Card>
