...
import { eventCategoryColors, SchoolEvent } from "../dto/event.dto";
import EventStatusControl from "./EventStatusControl";
import { deleteEvent } from "../hooks/use-event-mutations";
...
const initialFilters: EventFilter = { search: "", category: [], status: [] };
...
export default function EventList({
	...
	const [filter, setFilter] = useState<EventFilter>(initialFilters);
	const [page, setPage] = useState(1);
	const [limit, setLimit] = useState(10);
	const [deletingId, setDeletingId] = useState<string | null>(null);

	const { data, meta, isLoading } = useEvents({
		page,
		limit,
	...
	const columns: ColumnDef<SchoolEvent>[] = [
			...
			cell: ({ row }) => {
				...
				return (
					<div className="min-w-0">
						...
						{item.venue ? (
							<p className="text-muted-foreground truncate text-xs">{item.venue}</p>
						) : null}
					</div>
				);
			},
		},
		{
			...
			header: t("category"),
			cell: ({ row }) => {
				const colors = eventCategoryColors[row.original.category];
				return (
		...
		},
		{
			id: "status",
			header: t("status"),
			cell: ({ row }) => <EventStatusControl event={row.original} />,
		},
		{
			id: "actions",
			header: tc("actions"),
