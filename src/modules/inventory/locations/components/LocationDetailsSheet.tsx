...
export function LocationDetailsSheet({ id, open }: Props) {
	const t = useTranslations("Inventory");
	const { data: response, isLoading } = useLocation(open ? id : null);
				...
				{isLoading || !data ? (
					...
					<div className="space-y-4 p-4">
						<section className="bg-muted/20 rounded-md border p-4">
							<h3 className="text-sm font-normal">{t("locationInformation")}</h3>
							<div className="mt-4 grid grid-cols-1 gap-x-4 gap-y-4 @xl/body:grid-cols-2">
								<Pair label="Location Name" value={data.name} />
								<Pair label="Location Code" value={data.code} />
								<Pair label="Location Type" value={data.locationType} />
								<Pair label="Status" value={data.status === "ACTIVE" ? "Active" : "Inactive"} />
							</div>
						</section>
						<section className="bg-muted/20 rounded-md border p-4">
							<h3 className="text-sm font-normal">{t("classRoomInformation")}</h3>
							<div className="mt-4 grid grid-cols-1 gap-x-4 gap-y-4 @xl/body:grid-cols-2">
								<Pair label="Room No" value={data.classRoom?.roomNo} />
								<Pair label="Building" value={data.classRoom?.building} />
								<Pair
									label="Created Date"
